...
public class DrawChart {
    public static void drawChart(LineChart mLineChart, Pressure[] pressureArray,String note) {
        ...
        for (int i = 0; i < pressureArray.length; i++) {
            entries.add(new Entry((float) i, pressureArray[i].getPressure()));
        }

        //添加曲线说明
        LineDataSet lineDataSet = new LineDataSet(entries, note);//设置标注信息
        ...
        //设置y轴数据及样式
        setYAxisData(leftYAxis, rightYAxis, 0f, 100f);

        //在左侧y轴上绘制限制线
        drawLimitLine(leftYAxis, maxPressure, minPressure, "最大阈值", "最小阈值");
